...
void GenerateEvents(string OutputFileName, int NEvent, double EPythia, double EBall, double Mean, bool ExcludeE, int Shower);
...
int main(int argc, char *argv[])
{
   ...
   int Shower            = CL.GetInt("Shower", 1);
   bool ExcludeE         = CL.GetBool("ExcludeE", false);

   Assert(EPythia >= 10 || EPythia <= 0, "Pythia does not support E < 10");

   GenerateEvents(OutputFileName, N, EPythia, EBall, Mean, ExcludeE, Shower);

   return 0;
}

void GenerateEvents(string OutputFileName, int NEvent, double EPythia, double EBall, double Mean, bool ExcludeE, int Shower = 1)
{
   ...
   pythia.readString(Form("PartonShowers:model = %d", Shower));
   ...
   pythia.readString("Next:numberShowProcess = 2");        // do we print first hard process?  No!
   pythia.readString("Next:numberShowEvent = 2");          // do we print first event?  No!
